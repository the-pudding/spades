...
export default function forceCollideRect() {
  ...
  function force() {
    ...
    function iterate() {
      var quad = quadtree(nodes, xCenter, yCenter);
      var j = -1;
      while (++j < nodes.length) {
        ...
        quad.visit(collisionDetection);
      }
    }

    function collisionDetection(quad) {
      var updated = false;
      var data = quad.data;
      if (data) {
        if (data.index > node.index) {
          ...
          if (xDiff < 0 && yDiff < 0) {
            ...
            if (speed < 0) {
              ...
              var collisionImpulse =
                (2 * speed) / (masses[data.index] + masses[node.index]);
              if (Math.abs(xDiff) < Math.abs(yDiff)) {
                //x overlap is less
                data.vx -=
                  collisionImpulse * masses[node.index] * vCollisionNorm.x;
                node.vx +=
                  collisionImpulse * masses[data.index] * vCollisionNorm.x;
              } else {
                //y overlap is less
                data.vy -=
                  collisionImpulse * masses[node.index] * vCollisionNorm.y;
                node.vy +=
                  collisionImpulse * masses[data.index] * vCollisionNorm.y;
              }
